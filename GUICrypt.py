...
def pasteFromClipboard():
    try:
        tInput.ensert(END, rot.clipboard_get())
    except:
        tInput.insert(END, "\nОшибка: буфер пуст")
...
tInput.place(x=2, y=1)
tInput.insert(1.0, """Экземпляры Checkbutton также могут быть визуально оформлены в группы
но каждый флажок независим от остальных. Каждый может быть в состоянии
"установлен) или "снят", независимо от состояний других флажковю Другими словами, в группе Checkbutton можно сделать множественный выбор, в группе Radiobutton - нет.""")
scrollInput = Scrollbar(command=tInput.yview, width=20)
scrollInput.place(x=570, y=180, height=132)
...
menuInput.add_command(label="Вставить в исходный код", command=pasteFromClipboard)
...
rBtn = IntVar()
rBtn.set(0)
algo01 = Radiobutton (text="Ивертировать", variable=rBtn, value=0)
